...
The notebook **10 - Advanced inspection hypothesis test results.ipynb** makes it possible to perform the exploratory strategy described in the article's experimental evaluation on any auditable datasets. Assuming that there is a group of unfair autiable datasets, as well as the results of the assessment conducted on it by the notebook **7**, located in an appropriate path, the user can explore the assessment results and compare what the approach has found w.r.t. the true hotspots. The user can customize the notebook by changing the following variables: `name_set_groups_datasets`, which specifies the set of dataset groups considered; `idx_selected_group`, which specifies the dataset group selected from that set; and `idx_dataset`, which specifies the index of the dataset selected from the group.
