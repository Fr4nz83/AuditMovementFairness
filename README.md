...
- **9 - Plot results.ipynb**: this notebook generates the plots from the CSVs generated by the previous notebook. Please note that these are the plots shown in the **Figures 6, 7, 8, 9, and 10 in Section 6**. The notebook stores the plots in `./experiments/exp_plots` within the project's root folder. For each Figure there are three plots showing the results achieved when considering the power, sensitivity, and positive predictive value (PPV) metrics. Figure 6's plots' filenames are prefixed with `num_objects`, Figure 7's plots filenames with `mag_unfair`, Figure 8's plots filenames with `num_hotspots`, Figure 9's plots filenames with `num_regions`, and Figure 10's plots filenames with `num_stops`.
